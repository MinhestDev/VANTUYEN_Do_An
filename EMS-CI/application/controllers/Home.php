        ...
        if($check_login<>'')
        {
            if($check_login[0]['status']==1){
                ...
                elseif($check_login[0]['usertype']==2){
                    ...
                    $this->session->set_flashdata('login_error', 'Bạn không thể đăng nhập.', 300);
                    redirect(base_url().'login');
                }
                
            }
            else{
                $this->session->set_flashdata('login_error', 'Tài khoản của bạn đã bị cấm.', 300);
                redirect(base_url().'login');
            }
            
        }
        else{
            $this->session->set_flashdata('login_error', 'Hãy kiểm tra lại tài khoản hoặc mật khẩu.', 300);
            redirect(base_url().'login');
        }
    }
