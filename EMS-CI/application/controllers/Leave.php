...
class Leave extends CI_Controller {
    ...
    public function insert_approve($id)
    {
        ...
        {
            $this->session->set_flashdata('success', "Nghỉ phép được phê duyệt thành công"); 
        }else{
    ...
    public function insert_reject($id)
    {
        ...
        if($this->db->affected_rows() > 0)
        {
            $this->session->set_flashdata('success', "Đã bị từ chối"); 
        }else{
            $this->session->set_flashdata('error', "Lỗi.");
        }
        redirect($_SERVER['HTTP_REFERER']);
    ...
    public function update()
    {
        ...
        if($this->db->affected_rows() > 0)
        {
            ...
            $this->session->set_flashdata('error', "Cập nhật lỗi.");
        }
    ...
    function delete($id)
    {
        ...
        if($this->db->affected_rows() > 0)
        {
            ...
            $this->session->set_flashdata('error', "Xóa lỗi.");
        }
        redirect($_SERVER['HTTP_REFERER']);
    }
